...
var ListMissingIndexesCommand = (function () {
    function ListMissingIndexesCommand(db) {
        this.db = db;
        this.prefix = '.mindexes';
        this.usage = '.mindexes TABLE';
        this.description = 'Lists all the missing indexes on a table';
    }

    ListMissingIndexesCommand.prototype.run = function (messages, writer, args) {
        if (!validateArgs(args, 1)) {
            return Q.reject('Table name not specified');
        }
        
        return runQuery(messages, writer, this.db.query.bind(this.db, Queries.listMissingIndexesSql, [args[0]]));
    };
    return ListMissingIndexesCommand;
})();
...
function createAll(db) {
    var commands = [
            ...
            new ListMissingIndexesCommand(db),
            new QuitCommand()
        ];
